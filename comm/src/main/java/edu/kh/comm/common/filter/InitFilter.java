...
public class InitFilter extends HttpFilter implements Filter {
	...
	// Logger 객체 생성(해당 클래스에 대한 log출력하는 객체)
	//org.slf4j
	...
	public void init(FilterConfig fConfig) throws ServletException {
		...
		// debug : 개발의 흐름 파악 (이게 실행이 되었는지, 파라미터가 현재 무엇인지 확인할때)
		// info : 메소드 실행 파악
		
		// 이 필터가 생성되었다 라는 정보를 출력하고 싶다 : info
		logger.info("초기화 필터 생성");
	}
	
	// 필터가 파괴될 때 실행(서버는 켜져있는데 백엔드 코드가 수정되었을 때 필터 파괴)
	public void destroy() {
